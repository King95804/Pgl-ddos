...
bot = telebot.TeleBot('7205969548:AAEqrutTraxMdQynSLYLADZE31ervbMHYNI')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "madharchod Tu admin hai? 😂 Only Admin Can Run This Command 😡."
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"🚀 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃 🚀 : {username}\n\n🎯𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n🔗𝐏𝐨𝐫𝐭: : {port}\n⏰𝐓𝐢𝐦𝐞 : {time} Seconds\n⚙️𝐌𝐞𝐭𝐡𝐨𝐝: FREE ME OR KYA LEGA 😂 Send Feedback\n\n\nJOIN YOUR CHANNEL : https://t.me/Romeoddosavailable"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 180:
                ...
                response = f"✅BGMI ATTACK FINISHED✅ ABH MERI CHUT LE AUR NIKAL😅\n\nTarget : {target}\nPort : {port}\nTime : {time}\n\nᯓby: @t.me/Romeo804 𝐅𝐑𝐄𝐄 𝐃𝐃𝐎𝐒"
        else:
            response = "🤧 Usage :- /m4 <target> <port> <time>\nLAUD SHIDA CHUT ME DAAL BHEN CHOD DUNGI WARNA☠️"  # Updated command syntax
    else:
        response = "❌ Access defied dm- @KrishnaXcraxy ❌."
...
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''👋🏻Welcome to Your Home, {user_name}!\nFeel Free to Explore\n\nWELCOME TO THE @t.me/Romeo804 FREE DDOS SERVER FREEZE BOT'''
    bot.reply_to(message, response)
